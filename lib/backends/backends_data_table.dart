        ...
        return switch (state) {
          Success(:final backends) => BlocProvider(
              ...
              child: BlocBuilder<BackendsFilterBloc, BackendsFilterState>(
                builder: (context, state) {
                  return BlocListener<BackendsStatusUpdatesBloc,
                      BackendsStatusUpdatesState>(
                    listener: (context, state) {
                      switch (state) {
                        case Listening(:final statusUpdate):
                          final updatedBackends = backends.replaceFirstWhere(
                            (item) => item.name == statusUpdate.data.first.name,
                            (item) =>
                                item?.copyWith(
                                    queueLength:
                                        statusUpdate.data.first.lengthQueue) ??
                                backends.first,
                          );
                          context.read<BackendsFilterBloc>().add(
                              BackendsFilterEvent.backendsChanged(
                                  updatedBackends));
                          break;
                        default:
                          break;
